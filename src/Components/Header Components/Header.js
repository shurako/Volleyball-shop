...
window.addEventListener('resize', () => {props.setHeaderSize(header.current.clientHeight)})
...
const renderNav = () => {
    ...
    if(window.innerHeight > window.innerWidth ){
        return transitions((styles, item) => item ? <animated.div style = {styles} className={"nav-wrapper"}>
             <MainNav />
             <animated.div  className={"flex space-btw"}>
               {<Nav showMenu_mobile = {showMenu_mobile} setShowMenu_mobile = {setShowMenu_mobile}  />}
             </animated.div>
           </animated.div> : '')
...
const m_renderLoginSection = () => {
  ...
  return (
    <div style ={header__wrapper__style} ref = {header__wrapper} className={"header__wrapper"}>
      
      <div ref = {header} className={"header"}>
        {m_renderLoginSection()}  
        <HeaderMainSection />
        <div onClick = {() => {setShowMenu_mobile(!showMenu_mobile)} } className={"hamburger"}>
